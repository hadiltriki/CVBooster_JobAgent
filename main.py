...
async def api_ats_score(data: ATSRequest):
    ...
    cv_text = user.get("cv_raw_text", "")
    if not cv_text or not cv_text.strip():
        raise HTTPException(400, "CV non disponible. Relancez un scan d'abord.")
    ...
    # 3. Structurer CV + JD via GPT-4o (synchrone → thread)
    try:
        cv_data, jd_data = await asyncio.gather(
            asyncio.to_thread(structure_cv, cv_clean),
            asyncio.to_thread(structure_jd, data.job_description),
        )
    except ValueError as e:
